...
This guide contains manual quality assurance tests to make sure all the tools in this MCP server are functional on release. The automated suite (`bun test`) covers parsing, argument construction, and handler logic; these scenarios cover the real simulator integration that can't run in CI.

You can run a test case by copy and pasting it into a chat in an MCP client (like Cursor) that can run MCP tools.
...
1. Have the user open the native Photos app in the iOS simulator.
...
14. Call `ui_describe_all` to describe the page and see we are on the All Photos tab.
